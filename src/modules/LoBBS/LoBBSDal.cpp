...
std::vector<void *> LoBBSDal::getMailForUser(uint64_t userUuid, uint32_t offset, uint32_t limit)
{
    ...
    LOG_DEBUG("Retrieved %d mail messages for user " LODB_UUID_FMT " (offset=%d, limit=%d)", (int)result.size(),
...
{
    auto allNews = db->select("news", LoDbFilter(), LoDbComparator());
    ...
    LOG_DEBUG("Retrieved %d news items for user " LODB_UUID_FMT " (offset=%d, limit=%d)", (int)result.size(), LODB_UUID_ARGS(userUuid),
              offset, limit);
    return result;
}
