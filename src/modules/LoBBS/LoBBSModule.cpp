...
static bool loadUserByUuid(LoBBSDal *dal, uint64_t uuid,
                           meshtastic_LoBBSUser *outUser) {
...
static void truncateMessage(const char *message, char *buffer,
                            size_t bufferSize, size_t maxLen) {
...
ProcessMessage LoBBSModule::handleReceived(const meshtastic_MeshPacket &mp) {
  ...
  LOG_DEBUG("LoBBS received DM from=0x%0x, id=%d, msg=%.*s", mp.from, mp.id,
            mp.decoded.payload.size, mp.decoded.payload.bytes);
  ...
  if (isAuthenticated && msgBuffer[0] != '/' && strchr(msgBuffer, '@')) {
    ...
    while (*token) {
      if (*token == '@') {
        ...
        if (usernameLen > 0 && usernameLen < LOBBS_USERNAME_BUFFER_SIZE) {
          ...
          if (std::find(recipients.begin(), recipients.end(), usernameStr) ==
              recipients.end()) {
    ...
    if (foundAny && !recipients.empty()) {
      ...
      for (const auto &recipient : recipients) {
        ...
        // Send mail
        if (dal->sendMail(existingUser.uuid, recipientUuid,
                          messageContent.c_str())) {
          LOG_INFO("Sent mail from %s to %s", existingUser.username,
                   recipient.c_str());
          successCount++;
        } else {
      ...
      if (successCount > 0 && failCount == 0) {
        if (successCount == 1) {
          snprintf(replyBuffer, sizeof(replyBuffer), "Mail sent to @%s",
                   recipients[0].c_str());
        } else {
  ...
  if (strcasecmp(cmdName, "/hi") == 0) {
    ...
    if (userExists) {
      ...
      if (dal->verifyPassword(&existingUser, password)) {
        LOG_DEBUG("User %s authenticated successfully from node 0x%0x",
                  username, mp.from);
  ...
  if (strcasecmp(cmdName, "/users") == 0) {
    ...
    auto username_filter = [filterStr](const void *rec) -> bool {
      const meshtastic_LoBBSUser *u = (const meshtastic_LoBBSUser *)rec;
      return !filterStr || !filterStr[0] ||
             stristr(u->username, filterStr) != nullptr;
    };

    // Execute synchronous select with filter and sort
    auto users =
        dal->getDb()->select("users", username_filter, compareUsernames);
  ...
  if (strcasecmp(cmdName, "/mail") == 0) {
    ...
    // Determine action: list (default), list with offset, or read specific
    // message
    uint32_t offset = 0;
    int readMessageId = -1;
    ...
    auto mailMessages =
        dal->getMailForUser(existingUser.uuid, offset, MAIL_PAGE_SIZE);
    ...
    if (isReadCommand && readMessageId > 0) {
      ...
      const meshtastic_LoBBSMail *mail =
          (const meshtastic_LoBBSMail *)mailMessages[readMessageId - 1];
    ...
    for (size_t i = 0; i < mailMessages.size(); i++) {
      const meshtastic_LoBBSMail *mail =
          (const meshtastic_LoBBSMail *)mailMessages[i];
  ...
  if (strcasecmp(cmdName, "/news") == 0) {
    ...
    auto newsItems =
        dal->getNewsForUser(existingUser.uuid, offset, NEWS_PAGE_SIZE);
...
void LoBBSModule::sendReply(NodeNum to, const std::string &msg) {
  ...
  if (isTruncated) {
    size_t copyLen =
        MAX_REPLY_BYTES > truncMarkerLen ? MAX_REPLY_BYTES - truncMarkerLen : 0;
